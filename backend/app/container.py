...
class Container:
    def __init__(self):
        ...
        self.stock_search_service = StockSearchService(self.kis_client)
        self.news_client = NewsClient()
        self.embedding_client = EmbeddingClient()
        self.stock_collect_service = StockCollectService()
        self.stock_query_service = StockQueryService(self.kis_client, self.stock_search_service)
        self.news_embedding_service = NewsEmbeddingService(self.embedding_client)
        self.news_query_service = NewsQueryService(self.news_repository, self.news_embedding_repository, self.stock_repository, self.stock_search_service, self.embedding_client)
        self.news_collect_service = NewsCollectService(self.news_repository, self.news_embedding_repository, self.news_keyword_repository, self.stock_repository, self.news_client, self.stock_search_service, self.news_embedding_service)
        self.stock_tool = StockTool(self.stock_query_service, self.stock_search_service)
        self.news_tool = NewsTool(self.news_query_service, self.news_collect_service)
